...
%Example:
% load clouds
% t = ID3(patterns, targets, patterns, [5, 1]);
% disp(mean(t == targets))
...
%First, bin the data and the decision region data when neccessary
Nvalues           = zeros(1, Ni);
for i = 1:Ni,
    Nvalues(i)    = length(unique([train_patterns(i,:), test_patterns(i,:)]));
    if (Nvalues(i) > Nbins)
        [H, binned_train_patterns(i, :), region]  = high_histogram(train_patterns(i, :), Nbins);
        [H, binned_test_patterns(i,:) ]           = high_histogram(test_patterns(i,:), Nbins, region);
    else
        [a1, a2, binned_data]                     = unique([train_patterns(i,:), test_patterns(i,:)]);
        binned_train_patterns(i, :)               = binned_data(1:M);
        binned_test_patterns(i,:)                 = binned_data(M+1:end);
    end
end
