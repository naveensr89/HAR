function test_targets = Discrete_Bayes(train_patterns, train_targets, test_patterns, params)
...
%Example:
% load clouds
% patterns = round(patterns*5);
% t = Discrete_Bayes(patterns, targets, patterns, []);
% disp(mean(t == targets))
...
[unique_patterns, temp, pattern_indices] = unique(train_patterns', 'rows');
unique_patterns = unique_patterns';
N               = size(unique_patterns, 2);
counts          = zeros(length(Uc), N);
for i = 1:N
    counts(:, i) = hist(train_targets(pattern_indices == i), Uc)';
end
...
Pw_given_x = Px_given_w .* (p*ones(1,N));
