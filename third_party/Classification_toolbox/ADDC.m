function [patterns, targets, label, count] = ADDC(train_patterns, train_targets, Nmu, plot_on)
...
%	targets			- New targets
%   labels          - Labels for the original data points
%   count           - Counts of the online centroids
%
%See also k_means, LVQ1, LVQ3, BIMSEC
%
%Example:
% load clouds
% [new_patterns, new_targets, labels, count] = ADDC(patterns, targets, 10);
% disp(count)
...
for i = 1:L,
   ...
   if (K > 0),
      ...
      dist 				= sum((mu(:,1:K) - data * ones(1,K)).^2, 1);
      [temp, min_d]  = min(dist);
