...
%
%See also fuzzy_k_means, kernel_k_means, spectral_k_means
%
%Example:
% load clouds
% [new_patterns, new_targets, labels] = k_means(patterns, targets, 10);
% disp(new_patterns)
...
otherwise
   while (sum(sum(abs(mu - old_mu) > 1e-5)) > 0),
      ...
      for i = 1:Nmu,
         dist(i,:) = sum((train_patterns - mu(:,i)*ones(1,L)).^2, 1);
      end
