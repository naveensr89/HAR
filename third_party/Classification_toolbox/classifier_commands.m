...
case 'Start'
    ...
    [D, test_err, train_err, train_patterns, train_targets, reduced_patterns, reduced_targets, region] = ...
    ...
    %Recompute x and y in case they were changed
...
case 'FileNameInput'
    ...
    if (evalin('base', '~isempty(patterns)'))
        evalin('base',['region = calculate_region(patterns, [zeros(1,4) ' num2str(Npoints) ']);'])
    end
...
case 'ManualGraphData'
    ...
    evalin('base',['if ~(exist(''region'')), region = [-1 1 -1 1 ' num2str(Npoints) '];end']);
    evalin('base',['if (sum(abs(region(1:4))) == 0), region = [-1 1 -1 1 ' num2str(Npoints) '];end']);
