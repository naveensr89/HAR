...
%
%See also HDR, MDS, MultipleDiscriminantAnalysis, NLCA, PCA
%
%Example:
% load DHSchapter2
% [new_patterns, new_targets] = ICA(patterns, targets, [2, 0.1]);
...
patterns(find(~isfinite(patterns)))=0;
...
while (iter < 1000),
    ...
    %if (max(max(dW)) > 1e3),
    %    disp(['Algorithm diverged after ' num2str(iter) ' iterations'])
    %    break
    %end
