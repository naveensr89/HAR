function [D, test_err, train_err, train_patterns, train_targets, reduced_patterns, reduced_targets, region] = start_classify(patterns, targets, error_method, redraws, percent, Preprocessing_algorithm, PreprocessingParameters, Classification_algorithm, AlgorithmParameters, region, hm, SepratePreprocessing, plot_on)
...
for i = 1: redraws,  
   ...
   case {'PCA', 'Whitening_transform'}
       ...
       flatxy = uw*(flatxy - m*ones(1,size(flatxy,2)));
       ...
       if ~isempty(hParent),
           hold off
           plot_scatter(reduced_patterns, reduced_targets, hParent)
	       axis(region(1:4))
           hold on
       end
   ...
   case 'Scaling_transform'
       ...
       flatxy = (flatxy-m*ones(1,size(flatxy,2)))./(w * ones(1,size(flatxy,2)));
   ...
   case 'FishersLinearDiscriminant'
       ...
       reduced_patterns = [w'*train_patterns; zeros(1, size(train_patterns,2))];
       flatxy = [w'*flatxy; zeros(1, size(flatxy,2))];
