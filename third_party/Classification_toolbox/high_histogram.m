...
if (prod(size(Nbins)) == 1)
    Nbins = Nbins*ones(1, size(data,1));
end
...
for i = 1:size(data,1),
   ...
   %Cuase anything under the minimum to be in the lowest bin and everything
   %over the maximum to be in the highest bin
   spacing(1)   = -inf;
   spacing(end) = inf;
