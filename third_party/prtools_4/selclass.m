...
%       is a cell array of the desired classes
...
% a = gendatm; b = selclass(a);         % returns every class in a cell
% a = gendatb; 
% a = addlabels(a,genlab(25*ones(4,1)),'4class'); % add second label list
% b = selclass(a,[],'4class'); % returns 4 cells, preserves label list.
%
% SEE ALSO
...
function [b,J] = selclass(a,clas,lablist)
  ...
  if nargin < 2 | isempty(clas), clas = {}; end
  if nargin < 1 | isempty(a)
    b = mapping(mfilename,'fixed',{clas, lablist});
    return; 
  end
